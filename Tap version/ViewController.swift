...
final class ViewController: UIViewController, ARSCNViewDelegate,AVSpeechSynthesizerDelegate {
    ...
    private func updateOGMIfNeeded(time: TimeInterval) {
        ...
        stateQueue.sync {
            ...
            for det in sortedDet {
                ...
                // 2) 新規作成（上限まで）
                    ...
                    let samplePoint = self.preferredWorldSamplePoint(for: det)
                    guard let (wt, depth) = self.worldTransformAndDepthFirst(at: samplePoint),
                    ...
                    if self.isLikelyFloorPosition(worldPos, cameraTransform: frame.camera.transform) {
                        continue
                    }
    ...
    func findMatchingTrack(for det: Detection) -> (UUID, Track)? {
        let samplePoint = preferredWorldSamplePoint(for: det)
        // 検出の深さ（取れない時は nil）
        let detDepth: Float? = {
            if let (_, dd) = self.worldTransformAndDepthFirst(at: samplePoint) { return dd }
            return nil
        }()
    ...
    private func preferredWorldSamplePoint(for det: Detection) -> CGPoint {
    ...
    private func isLikelyFloorPosition(_ worldPos: simd_float3, cameraTransform: simd_float4x4) -> Bool {
